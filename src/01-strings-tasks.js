...
function removeLeadingAndTrailingWhitespaces(value) {
  return value.includes('\'') ? value.slice(value.indexOf('\'') + 1, value.length) : value.trim();
}
...
function getRectangleString(width, height) {
  ...
  for (let i = 0; i < height - 2; i++) {
    result.push(`${v + ' '.repeat(width - 2) + v}\n`);
  }
  return `${one + h.repeat(width - 2) + two}\n${result.join('')}${three}${h.repeat(width - 2)}${four}\n`;
}
